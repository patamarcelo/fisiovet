...
export default function TutorForm() {
	...
	const validation = useMemo(() => {
		...
		const phoneOk = !phoneDigits || (phoneDigits.length >= 10 && phoneDigits.length <= 11);
	...
	const firstError = useMemo(() => {
		...
		if (!validation.phoneOk) return "Telefone opcional, mas precisa ter 10–11 dígitos com DDD quando preenchido.";
		if (!validation.emailOk) return "E-mail inválido.";
		if (!validation.cepOk) return "CEP opcional, mas precisa ter 8 dígitos quando preenchido.";
		if (!validation.ufOk) return "UF opcional, mas precisa ter 2 letras quando preenchida.";
		return null;
	}, [validation]);
	...
	const onChangeCep = useCallback(async (value) => {
		...
		try {
			...
			Alert.alert(
				"CEP",
				err?.message || "Não foi possível buscar o CEP. Você pode preencher o endereço manualmente."
			);
		} finally {
			setLoadingCep(false);
		}
	...
	const onSubmit = useCallback(async () => {
		...
		try {
			...
			const shouldTryGeocode =
				hasAnyAddressField(enderecoBase) && hasMinimumAddressForGeocode(enderecoBase);
			...
			if (shouldTryGeocode) {
				try {
					...
					geoEnriched = {
						...
						precision: geo?.precision || geo?.raw?.geometry?.location_type || null,
						types: geo?.raw?.types || [],
						viewport: vp
							? {
								northeast: {
				...
				>
					...
					>
						...
						<View style={styles.twoColumns}>
							...
							<View style={{ flex: 0.65 }}>
								<Field label="UF" error={!validation.ufOk ? firstError : null}>
									<ThemedTextInput
										placeholder="UF"
										ref={ufRef}
										value={uf}
					...
					{!!firstError && (
						<Text
							style={[
								styles.bottomHint,
								{ color: validation.nameOk ? subtle : danger },
							]}
						>
							{firstError}
						</Text>
